  ...
  if (/.*\.(jpg|gif|png|jpeg|tiff|heif|bmp|webp)$/i.test(msg)) {
    return (
      <img
        ...
        className="max max-w-[350px] max-h-[500px] rounded-lg drop-shadow-md"
      />
    );
  } else {
    return (
      <p className="p-3 rounded-lg bg-slate-100 text-black break-word">{msg}</p>
    );
  }
}
  ...
  return (
    <main className="text-black pb-5 md:w-8/12 md:mx-auto w-full flex flex-col md:flex-row justify-center items-stretch gap-5 h-[calc(100dvh - 100px)]">
      ...
      <section className="w-full" style={{ flex: 1.5, height: "100%" }}>
        <Suspense fallback={<div className="w-full h-full flex justify-center items-center"><p className="text-2xl font-bold">Cargando...</p></div>}>
